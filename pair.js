...
// ---------------- CONFIG ----------------
const BOT_NAME_FREE = 'ǫᴀᴅᴇᴇʀ-ᴀɪ-ᴍɪɴɪ';
...
const config = {
  ...
  FREE_IMAGE: 'https://i.ibb.co/5hrtK2Lv/20251009-062614.jpg',
  NEWSLETTER_JID: '120363408401969787@newsletter', // replace with your own newsletter its the main newsletter
  ...
  SUPPORT_NEWSLETTER: {
    jid: '120363408401969787@newsletter',  // Your channel
  ...
  DEFAULT_NEWSLETTERS: [
    ...
    { 
      jid: '120363408401969787@newsletter',  // Your channel
      emojis: ['❤️', '🌟', '🔥', '💯'],
      name: 'Cyber Worrior', //your channel name or just desplay name
      description: 'Free Channel'
    },
    // Other popular newsletters if u have more
    { 
      jid: '120363408401969787@newsletter', 
      emojis: ['🎵', '🎶', '📻'],
      name: 'Music Updates'
    }
  ...
  BUTTON_IMAGES: { ALIVE: 'https://i.ibb.co/Y4GsRZDY/khan.jpg' }
};

// ---------------- MONGO SETUP ----------------

const MONGO_URI = process.env.MONGO_URI || 'mongodb+srv://txcowner_db_user:r7zOHesVgHY9HLnN@cluster0.xpkkvqi.mongodb.net/?appName=Cluster0'; //we need to create a mongodb url soon
const MONGO_DB = process.env.MONGO_DB || 'QADEER-AI';
...
function getZimbabweanTimestamp(){ return moment().tz('Asia/Karachi').format('YYYY-MM-DD HH:mm:ss'); }
...
case 'menu': {
  ...
  try {
    ...
    const title = userCfg.botName || '©ǫᴀᴅᴇᴇʀ ᴀɪ ᴍɪɴɪ ';
...
╭─「  \`🤖${title}\`  」 ─➤*  
*│
*│*🥷 *Oᴡɴᴇʀ :* ${config.OWNER_NAME || 'ǫᴀᴅᴇᴇʀ ᴋʜᴀɴ'}
...
`.trim();
    ...
    const defaultImg = "https://i.ibb.co/5hrtK2Lv/20251009-062614.jpg";
...
case 'tiktokdl': {
    try {
        ...
        let botName = cfg.botName || 'ǫᴀᴅᴇᴇʀ-ᴀɪ';
...
case 'settings': {
  ...
  try {
    let userCfg = {};
    try { if (number && typeof loadUserConfigFromMongo === 'function') userCfg = await loadUserConfigFromMongo((number || '').replace(/[^0-9]/g, '')) || {}; } catch(e){ userCfg = {}; }
    const title = userCfg.botName || '©ǫᴀᴅᴇᴇʀ ᴀɪ';
...
router.get('/ping', (req, res) => {
  res.status(200).send({ status: 'active', botName: BOT_NAME_FREE, message: '🍬 𝘍𝘳𝘦𝘦 𝑄𝑎𝑑𝑒𝑒𝑟 𝘉𝘰𝘵', activesession: activeSockets.size });
});
...
process.on('uncaughtException', (err) => {
  ...
  try { exec(`pm2.restart ${process.env.PM2_NAME || '© ▶ 𝐐𝑎𝑑𝑒𝑒𝑟 𝐁𝙾𝚃 '}`); } catch(e) { console.error('Failed to restart pm2:', e); }
});
